...
;(function($) {
  ...
  $.fn.tw_city_selector = function(selectFirst, selectSecond, selectThird) {
    ...
    var selectFirstPrepend = '<option value="">選擇縣市</option>',
        selectSecondEmptyPrepend = '<option value="">---</option>',
        selectSecondActivePrepend = '<option value="">選擇區域</option>',
    ...
    Selector.prototype.init = function() {
      ...
      for (var i = 0, j = country.length; i < j; i++) {
        if ( typeof custom === 'object' && $.inArray(country[i], custom) === -1 ) {
          ...
          // format: <option value="台北市" data-order="0">台北市</option>
      ...
      // 區域選項內容
      // ---------------
      $selectSecond.prepend(selectSecondEmptyPrepend);
    };
    ...
    Selector.prototype.countryChange = function() {
      ...
      $selectFirst.change(function() {
        ...
        if (order !== undefined) { // 選擇有值的選項
          $selectSecond.prepend(selectSecondActivePrepend);
